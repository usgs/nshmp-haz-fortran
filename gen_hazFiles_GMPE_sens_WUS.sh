...
#WUS
  for modf in `ls $faults_dir | grep _${gmpenm}`; do
    if (( $log_run_haz_check )); then
      echo $bin_dir/hazFXnga7.temp $faults_dir_rel/$modf 
    fi
    if (( $log_run_haz )); then
      logf=`echo $modf | sed 's/\.in//'`
      $bin_dir/hazFXnga7.temp $faults_dir_rel/$modf >& $log_dir/${logf}.log
    fi
  done
#CA
  for modf in `ls $faults_dirCA | grep _${gmpenm}`; do
    if (( $log_run_haz_check )); then
      echo $bin_dir/hazFXnga7.temp $faults_dir_relCA/$modf 
    fi
    if (( $log_run_haz )); then
      logf=`echo $modf | sed 's/\.in//'`
      $bin_dir/hazFXnga7.temp $faults_dir_relCA/$modf >& $log_dir/${logf}.log
    fi
  done
# gridded files
