c----program hazallXL.v4.f Last rev Mar 3 2011. Stephen Harmsen
c Mar 2011: THIS CODE is probably WORKING RIGHT. tested around Island of Fiji and greater
c SW Pacific region.
c Solaris:
c f95 hazallXL.v4.f iosubs.o -e -o hazallXL.v4
c    -e to extend line length 
c Linix cluster, gldlabm:
c	ifort hazallXL.v4.f iosubs.o -O -132 -o ../Bin/hazallXL.v4
c What about -static flag? or -i-static (Intel lib routines)
c
c based on hazallXL.f (frankel). THe big gain in v4 is increased flexibility of
...
c
c usage hazallXL.v4 input.file 
...
c--- to run: hazallXL.v4  control.script 
...
c --- v4 : first two lines of control file describe the grid of sites to output.
...
c versions only output a given gm index. This version interpolates (ln-ln).
      parameter (nfmax=60,tinytim=1.e-20)   !max number of input files
      type header 
      ...
      logical ok,new_model,vsmatch/.true./,first,ascii,lastread
      logical, dimension(nfmax) :: eof
...
c      real dx,dy,scale,gymax,gymin,gxmax,gxmin,gdx,gdy,tiny/1.e-12/,tinytim
      real dx,dy,scale,gymax,gymin,gxmax,gxmin,gdx,gdy,tiny/1.e-12/
c      pointer (pp1,prob1),(pp2,prob)
c      real, dimension(*):: prob, prob1
      real, dimension(200000000):: prob, prob1
...
c
	...
	kstart=1	!starting block of reads
      nom_de_plume=name(1)
       print *,"This program is hazallXL.v4 (Harmsen)"
       print *,'Different W&E long and N&S lat ranges are OK. All data must be inside master grid'
...
50      format(a,$)
 ...
 900  format(a)
      ...
      if(ok)then
      call openr(name(ifil))
      else
      print *,name(ifil),' not found. Fix your script.'
      stop 'hazallXL.v4 did not finish'
      endif
c 30 character name
c      nhd = 308
...
c------ get or check map parameters
      if(ifil.lt.7)headr%name(ifil)= name(ifil)(1:30)
       nlev= head%nlev
      xmin=head%extra(2) 
      ymin=head%extra(5) 
      xmax=head%extra(3)
      ...
      dx=dxh; dy=dyh
        ...
        if(dxh.ne.gdx)stop 'dx of file does not match dx of output. Unacceptable'
        if(dyh.ne.gdy)stop 'dy of file does not match dy of output. Unacceptable'
       if(ifil.eq.1)then
...
c set xlev based on 1st input file.
      ...
      xlev(i)= head%xlev(i)
      headr%xlev(i)=xlev(i)
1      ylev(i)=alog(xlev(i))
	vs30o=head%extra(9)
	ndataout=nxout*nyout*nlev
c allocate space for output
c		pp2=malloc(ndataout*4)
c initialize. Note explicit mention of all elements
c	  prob(kk) = 1.e-21	!initialize. Note explicit mention of all elements
	prob = 1.e-21	
        do i=1,10
...
c are worthy of note but not different enough to shut down the process.
      ...
      enddo
       ...
       write(6,5) "Enter weight for these hazard curves: "
      read(infi,*) wt(ifil)
      write(6,*)wt(ifil)
	weight=wt(ifil)
...
32      format('#Pgm hazallXL.v4.f (harmsen) sums ',i2,' hazard curves from ',
     + a,/,'#Lat Long   Rex for spectral period ',f4.2,/,'#GM set(g)',i3)      
      ...
      open(3,file=namout,status='unknown')
33      format('#Pgm hazallXL.v4.f (harmsen) sums ',i2,' hazard curves. Site Vs30',f6.1,
     + /,'#Long    Lat  SA(g) for spectral period ',f4.2,' Annual Rex ',1pe11.5) 
34      format('#Pgm hazallXL.v4.f (harmsen) sums ',i2,' hazard curves. site Vs30',f6.1,
     + /,'#Long    Lat  PGV(cm/s) or PGD for index ',f4.1,' Rex ',1pe11.5) 
35      format('#Pgm hazallXL.v4.f (harmsen) sums ',i2,' hazard curves. Site Vs30',f6.1,
