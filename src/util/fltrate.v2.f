c-----fltrate.v2.f: calculates rates for characteristic EQ's and
...
c f95 fltrate.v2.f -e -o ../bin/fltrate.v2
c gnu fortran:
c  gfortran fltrate.v2.f -ffixed-line-length-none -finit-local-zero -ffpe-trap= -o fltrate.v2
...
c ic = number of faults where mmax is determined from area or length
      real magmax,magmin,magmin2,magmax0,morate,momax,momin
      real cmag,cmagmax,az,azo
      real mmax(3)
 900  format(a)
 50      format(a,$)
       if(iargc().gt.0)then
...
1      write(6,50) "Enter name of input file: "
      ...
      read(5,*) iuplift
      write(6,50) "enter name of output file for log: "
      read(5,900) namelog
...
1009      format('#Xlen width Mmax(area) ID Name of fault')
      ...
      write(20,900)'#ID,name,Length(km),M_char,Recurr_char,weight,A_GR,B_GR,M_min,MMax,Delta_M'
       read(1,900)nameflt
...
c      write(20,900)nameflt
      ...
      stop 'fltrate.v2: please check input'
      endif
...
c effectively kills this fault-specific information. SH may 3 2006.      
      if(sdep.ne.0.) width= sdep/sin(abs(dip)*coef)
      if((iuplift.eq.1).and.(dip.lt.89.).and.itype.gt.1) rate= rate/sin(abs(dip)*coef)
...
c Here is the primary v2 change: discretize Mmax to 0.01
      ...
      imag= xmag+0.5
      xmag= float(imag)/100.
      if(itype.eq.3)write(10,1001)xlen,width,xmag,nameflt
...
c from dmag/2 to shift, defined above.
      ...
      write(3,*) cmag,crate,wt,morate
      write(3,*) dip, width, top, xlen
      write(3,*) npts
      write(20,202)nameflt(1:20),xlen,cmag,1./crate,wt,a,b,magmin,magmax,dmaggr
      do 100 i=1,npts
 ...
 905  format(f14.5,1x,f14.5)
      ...
      write(3,*) a2,b,ymag,magmax,dmag,wt      !ymag replaces magmin for char
      arate=10**(a2-b*magmax)
      write(20,202)nameflt(1:20),xlen,magmax,1./arate,wt,a,b,magmin,magmax,dmaggr
202      format(a20,1x,f6.1,1x,f5.2,1x,f8.1,1x,f5.3,1x,f8.5,1x,f5.2,1x,f5.2,1x,f5.2,1x,f6.4)
      write(3,*) dip, width, top, xlen
      write(3,*) npts
      do 101 i=1,npts
...
c precompute tables of Pex.
      write(4,444) a,b,magmin,magmax,dmaggr,wt,morate
444     format(f8.5,1x,f5.2,1x,f6.3,1x,f6.3,1x,f6.4,1x,f7.5,1x,e11.5)
      write(4,*) dip, width, depth, xlen
      write(4,*) npts
      do 106 i=1,npts
 106  write(4,905) y(i),x(i)
      ...
      write(8,*) cmag,crate,wt,morate
      write(8,*) dip, width, depth, xlen
      write(20,202)nameflt(1:20),xlen,cmag,1./crate,wt,0.,b,0.,0,0.
      write(8,*) npts
      do 105 i=1,npts
 105  write(8,905) y(i),x(i)
        endif
      ctime= 1./crate
      write(2,*) cmag,crate,ctime,xlen,wt
      write(2,*) a,b,magmin,magmax,dmag,morate,xmagl
      else
