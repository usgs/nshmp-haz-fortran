c assim.2013.f
c modified feb 10 2014 to downweigh AB03 subduction to 0.1
...
c
	...
	logical written/.false./,usewt,usewtc,subduction,more,wus13,namef
c sa is the median sa for a given atten model and sd is the std dev. for that model. Assume namx models
	...
	indc2 = (/5,7,20,21/)
	wtc2 = (/0.1,0.3,0.3,0.3/)	! revised weights to cascadia GMMs feb 10 2014
...
4	sabar=0.0
	...
	goto 4
2	if(namef)then
	write(2,222)x,y,xmagmax,rjbmax,rrupmax,sbmax/namx,(samax(k),k=1,namx),
     + (sdmax(k),k=1,namx),fname(ifltmx)
222	format(	f8.2,1x,f6.2,1x,f7.2,1x,f7.2,1x,f7.2,<ic>(1x,e11.5),1x,a)
	else
