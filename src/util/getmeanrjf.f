c compute mean distance from fault with random strike,
c where distance is measured from its midpoint
c Use Wells and C. (1) SRL or (2) Area for WUS, Somerville A for CEUS
c WC Area:
c West:		0.91M-3.49  Zmax=15
c East: 	M-4.366		Zmax=20 km
c
	logical ishere
	character*18 fn(3)/'rjbmean.bin.srl','rjbmean.bin.Awest','rjbmean.bin.Aeast'/
	character*18 fa(3)/'rjbmean.dat.srl','rjbmean.dat.Awest','rjbmean.dat.Aeast'/
        common/rjbmean/rjbmean(16,1001)
        real rjbb(16,1001)
	real mmin,mmax,dm
	mmin=6.05
	mmax=7.55
...
1	write (6,50)'Enter 1 for WC SRL, 2 for WC WUS A, 3 for Somerville E: '
	read *,itype
50	format(a,$)
	if(itype.lt.1.or.itype.gt.3)goto 1
...
10	format(/,'#Mag ',f6.2,/,'#Dcenter(km) Dbar(km) from getmeanrjf')
	do ir=1,nr
	if(ir.lt.201)write(2,12)r,	rjbmean(im,ir)
